...
(function(){
  ...
  function uniqueTexts(pages){
    const texts=pages.map(p=>String(typeof p==="string"?p:(p&&p.text)||"").trim().toLowerCase()).filter(Boolean);
    ...
    [["story",lesson.story],["reader1",lesson.reader1],["reader2",lesson.reader2]].forEach(([key,reading])=>{
      ...
      specs.forEach((spec,pageIndex)=>{
        ...
        (spec.props||[]).forEach(prop=>{
          if(propKinds.size&&!propKinds.has(prop.kind)) add(id,"error",`${key} page ${pageIndex+1} uses unknown prop: ${prop.kind}.`);
        });
      });
    });

    const phonics=lesson.phonics||{};
    if(!Array.isArray(phonics.examples)||phonics.examples.length<4) add(id,"warning","Phonics should provide at least four examples when teaching two letters.");
  ...
  if(errors.length) console.error("Fritz Week 1 quality gate failed",window.FritzWeek1QualityReport);
  else console.info("Fritz Week 1 quality gate passed",window.FritzWeek1QualityReport);
})();