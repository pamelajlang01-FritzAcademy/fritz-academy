...
(function(){
  ...
  function install(){
    ...
    document.addEventListener("pointermove",e=>{
      ...
      const hx=Math.max(8,item.offsetWidth*0.43/r.width*100),hy=Math.max(10,item.offsetHeight*0.43/r.height*100);
    ...
    document.addEventListener("pointerup",()=>{
      ...
      stage.querySelectorAll(".fritz-builder-object").forEach(item=>{
        const r=stage.getBoundingClientRect(),hx=Math.max(8,item.offsetWidth*.43/r.width*100),hy=Math.max(10,item.offsetHeight*.43/r.height*100);
