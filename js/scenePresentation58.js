/* Fritz Academy Scene Presentation v63
   Approved student cadets + complete named cast + story-specific visible props. */
(function(){
  'use strict';
  ...
  const CAST={
    'Captain Fritz':{src:CAST_RAW+'assets/characters/approved/captain-fritz/nautical-idle.webp',fallback:ART_RAW+'assets/captain_fritz.png',scale:.92},
    'Bash':{src:CAST_RAW+'assets/characters/approved/bash/sweatsuit-idle.webp',fallback:ART_RAW+'assets/characters/bash/standing.png',scale:1.04},
    'Bear':{src:CAST_RAW+'assets/characters/approved/bear/sweatsuit-idle.webp',fallback:ART_RAW+'assets/characters/bear/standing.png',scale:.76},
    'Nola':{src:CAST_RAW+'assets/characters/approved/nola/sweatsuit-idle.webp',fallback:ART_RAW+'assets/characters/nola/standing.png',scale:.82},
    'Tony':{src:CAST_RAW+'assets/characters/approved/tony/sweatsuit-idle.webp',fallback:ART_RAW+'assets/characters/tony/standing.png',scale:.56},
    'Rascal':{src:'assets/characters/approved/rascal/approved-hero.webp',fallback:'assets/characters/approved/rascal/approved-hero.webp',scale:.72}
  };
  const ENV={
  ...
  const PROP={
    ...
    ball:'assets/objects/story-ball.svg',cat:'assets/objects/story-cat.svg'
  };
  ...
  function propsFor(text){
    ...
    const add=(test,src,kind='normal')=>{if(test&&!arr.some(x=>x.src===src))arr.push({src,kind});};
    add(t.includes('ball'),PROP.ball,'ball');add(t.includes('cat'),PROP.cat,'cat');
  ...
  function injectStyles(){
    if(document.getElementById('fa-scene63-style'))return;
    const s=document.createElement('style');s.id='fa-scene63-style';s.textContent=`
      ...
      .fa-scene58-prop{position:absolute;object-fit:contain;filter:drop-shadow(0 6px 5px rgba(0,0,0,.28));z-index:4;pointer-events:none}.fa-scene58-prop.p0{width:14%;left:6%;bottom:1%}.fa-scene58-prop.p1{width:13%;right:7%;bottom:2%}.fa-scene58-prop.p2{width:11%;left:25%;bottom:0}.fa-scene58-prop.p3{width:11%;right:26%;bottom:0}.fa-scene58-prop.p4{width:10%;left:46%;bottom:0}.fa-scene58-prop.p5{width:10%;right:42%;bottom:0}
      .fa-scene58-prop.ball{width:8%!important;right:18%!important;left:auto!important;bottom:2%!important;z-index:5!important}.fa-scene58-prop.cat{width:9%!important;right:5%!important;left:auto!important;bottom:46%!important;z-index:5!important}
  ...
  function open(opts){
    ...
    propsFor(opts.text).forEach((prop,i)=>{const im=document.createElement('img');im.className='fa-scene58-prop p'+i+' '+prop.kind;im.src=prop.src;stage.appendChild(im);});
    ...
    names.forEach(name=>{const spec=CAST[name],img=document.createElement('img');img.className='fa-scene58-char';img.src=spec.src;img.alt=name;img.title=name;img.dataset.name=name;img.style.transform=`scale(${spec.scale})`;img.dataset.fallback='0';img.onerror=()=>{if(img.dataset.fallback==='0'&&spec.fallback){img.dataset.fallback='1';img.src=spec.fallback;}else{img.remove();}};cast.appendChild(img);});
