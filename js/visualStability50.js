/* Fritz Academy visual stability patch v50.1 — preserve canonical illustrated environments */
(function(){
  "use strict";
  ...
  function addFallbackBackground(scene,objects,x,y,w,h,id){
    const palette={"academy-front":[0x9bdcff,0x8bd06f],"welcome-garden":[0xb8e8ff,0x79c96a],"reading-room":[0xffefd2,0xd7a86e],classroom:[0xe8f3ff,0xc7d8ee],"music-room":[0xffe8f2,0xd8b6df],playground:[0xa9e0ff,0x8bd06f],campus:[0xb6e7ff,0x7bc86b]}[id]||[0xb6e7ff,0x7bc86b];
    objects.push(scene.add.rectangle(x,y-h*.14,w,h*.72,palette[0]).setDepth(1));
    objects.push(scene.add.rectangle(x,y+h*.30,w,h*.38,palette[1]).setDepth(1));
  }
  if(typeof IllustrationEngine!=="undefined"){
    IllustrationEngine.prototype.drawScene=function(objects,text,options={}){
      ...
      if(envKey&&this.scene.textures.exists(envKey)){
        objects.push(this.scene.add.image(x,y,envKey).setDisplaySize(w-8,h-8).setDepth(1));
      }else{
        console.warn("Fritz Academy illustrated environment unavailable; using fallback:",env&&env.id);
        addFallbackBackground(this.scene,objects,x,y,w-8,h-8,env?env.id:"campus");
      }
