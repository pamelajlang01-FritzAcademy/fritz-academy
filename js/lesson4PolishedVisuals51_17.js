...
(function(){
  ...
  const pairs=[
    ["tony","bash"],["bear","nola"],["nola","fritz"],
    ["rascal","fritz"],["bash","fritz"],["tony","fritz"]
  ];
  ...
  function render(host,section,index,onNext,nextLabel){
    ...
    art.innerHTML=`<span class="l4p-cloud" style="left:8%"></span><span class="l4p-cloud" style="right:8%;transform:scale(.75)"></span><span class="l4p-tree" style="left:7%"></span><span class="l4p-tree" style="right:7%;transform:scale(.85)"></span><div class="l4p-badge" style="border-color:${accent}">${words[i]}?</div><div class="l4p-bubble">${page.text?host.lessonEngine.replaceName(page.text).split('.').slice(0,1).join('.'):'Ask a good question.'}</div><img class="l4p-character l4p-left" src="${chars[p[0]]}" alt=""><img class="l4p-character l4p-right" src="${chars[p[1]]}" alt="">`;
  ...
  if(window.BuilderEngine){
    ...
    BuilderEngine.prototype.showBuilder=function(){
      if(this.build&&this.build.areaId){
        this.build.backgroundImage='assets/environments/question-garden-premium.svg';
      }
