...
(function(){
  ...
  ids.forEach((id,index)=>{
    ...
    [["story",lesson.story],["reader1",lesson.reader1],["reader2",lesson.reader2]].forEach(([key,reading])=>{
      const pages=reading&&Array.isArray(reading.pages)?reading.pages:[];
      const specs=scenes[id]&&Array.isArray(scenes[id][key])?scenes[id][key]:[];
      if(pages.length<5)add(id,"error",`${key} needs at least five pages.`);
      if(specs.length!==pages.length)add(id,"error",`${key} illustration count does not match page count.`);
    });
    ...
    const next=index===0?"3-B":"3-C";
    if(!lesson.completion||lesson.completion.unlocks!==next)add(id,"error",`Lesson must unlock ${next}.`);
  });
  const errors=reports.filter(r=>r.severity==="error");
  window.FritzWeek3QualityReport={version:"48.0",passed:errors.length===0,errors,all:reports,checkedAt:new Date().toISOString()};
